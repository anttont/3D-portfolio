    ...
    <div className='xl:mt-12 xl:flex-row flex-col-reverse gap-10 overflow-hidden'>
      <motion.div variants={slideIn("left", "tween", 0.2, 1)} className='w-full green-pink-gradient p-[2px] p-8 rounded-2xl'>
        ...
        {submitted ? (
          ...
          <p className={`${styles.sectionHeadText} text-white`}>{language === 'en' ? 'Get in touch' : 'Ota yhteyttä'}</p>)}
          {submitted ? (
          ...
          {submitted ? (
            ...
            <p className={`${styles.sectionSubText} text-white mb-4`}>
              {language === 'en' ? 'I will get to you as soon as possible.' : 'Otan teihin yhteyttä mahdollisimman pian.'}
            </p>
            
            <button onClick={handleReturn} className='bg-[#131314] py-3 px-8 outline-none w-fit text-fit font-bold shadow-md shadow-primary rounded-xl'>
            ...
            >
              ...
              <button type="submit" className='bg-[#131314] py-3 px-8 outline-none w-fit text-fit font-bold shadow-md shadow-primary rounded-xl'>
                {loading 
                  ? language === 'en'
                    ? 'Sending...'
                    : 'Lähetetään...'
